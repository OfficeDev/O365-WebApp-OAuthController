...
namespace O365_WebApp_OAuthController.Controllers
{
    public class OAuthController : Controller
    {
        ...
        // It is not intended to be called directly, only as a redirect from the authorization request in FilesController.
        ...
        public ActionResult Index(string code, string error, string error_description, string resource, string state)
        {
            ...
            try
            {
                ...
                AuthenticationContext authContext = new AuthenticationContext(SettingsHelper.Authority, new TokenDbCache(userObjectID));
                AuthenticationResult result = authContext.AcquireTokenByAuthorizationCode(
                    code, new Uri(Request.Url.GetLeftPart(UriPartial.Path)), credential, SettingsHelper.AADGraphResourceId);
            ...
            {
                return Redirect("/Files/Index?authError=token");
            }
        }
